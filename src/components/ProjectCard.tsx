...
const ProjectCard: React.FC<IProjectCardProps> = ({ project }) => {
  ...
  return (
    <motion.div
      className="group relative bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 border border-clay/20 hover:border-clay/60 backdrop-blur-sm hover-lift"
      ...
      <div className="p-6 relative z-10">
        ...
        <div className="mb-4 flex flex-wrap gap-2">
          {project.technologies.map((tech, index) => (
            <motion.span
              key={index}
              className="text-xs font-bold px-3 py-1.5 rounded-full bg-gradient-to-r from-clay/20 to-ivory/20 text-clay border border-clay/40 group-hover:border-clay group-hover:from-clay/40 group-hover:to-ivory/40 transition-all duration-300 uppercase tracking-wider"
        ...
        <div className="flex gap-3 relative z-20">
          <motion.a
            ...
            className="flex-1 relative px-4 py-3 bg-gray-800 border border-clay/40 text-white rounded-lg flex items-center justify-center gap-2 group/btn font-bold text-sm transition-all duration-300 overflow-hidden"
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
          >
            <div className="absolute inset-0 bg-gradient-to-r from-clay/0 to-clay/30 transform translate-x-[-100%] group-hover/btn:translate-x-0 transition-transform duration-500"></div>
